...
def step_add_to_cart(context):
    ...
    assert product, "Aucun produit sélectionné — utilisez 'je suis sur la page produit de'"
...
def step_check_product_qty(context, search_name, expected_qty):
    product = find_product(context, search_name)
    item = next(
        (i for i in context.cart if i["name"] == product["name"]), None)
...
def step_check_price_matches_catalog(context):
    ...
    assert product["price"] == catalog_price, (
